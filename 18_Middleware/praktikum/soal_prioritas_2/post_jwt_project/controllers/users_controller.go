...
func Register(ctx echo.Context) error {
	var request model_post_jwt.UserRequest
	...
	newUser := model_post_jwt.User{
		Email:    request.Email,
		Password: password,
	}

	if err := config.DB.Create(&newUser).Error; err != nil {
		return ctx.JSON(http.StatusBadRequest, errorHandler_posts_jwt.ErrorHandler{
			Status:     false,
	...
	response := model_post_jwt.UserResponse{
		Status:     true,
...
func Login(ctx echo.Context) error {
	...
	token, err := middleware_posts.GenerateToken(&user)
	if err != nil {
		return ctx.JSON(http.StatusInternalServerError, errorHandler_posts_jwt.ErrorHandler{
			Status:     false,
			StatusCode: http.StatusInternalServerError,
	...
	response := model_post_jwt.LoginResponse{
		Status:     true,
